...
from modules.qwen_modules import QwenConfig
from modules.tokenizer import build_tokenizer_qwen
from pipeline_parallel.dist_gpipe_qwen_pipeline_async import GpipeAsync
from pipeline_parallel.dist_pp_qwen_utils import get_pp_module
...
def main():
    ...
    config = QwenConfig.from_pretrained(args.model_name)
    ...
    tokenizer = build_tokenizer_qwen(args)
            ...
            if get_pipeline_parallel_rank() == 0:
                ...
                for i in range(len(pipe.model.model)-1):
                    ...
                    pipe.model.model[i].load_state_dict(
                        torch.load(f'{load_path}/pytorch_{_i + i}.pt')
                    )
                pipe.model.model[-1].load_state_dict(
                    torch.load(f'{load_path}/pytorch_lm_head.pt')
                )
            else:
